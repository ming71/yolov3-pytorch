...
def train():
    ...
    epochs = 1 if opt.prebias else opt.epochs  # 500200 batches at bs 64, 117263 images = 273 epoch  迁移学习只需要简单的预训练
    batch_size = opt.batch_size
    accumulate = opt.accumulate  # effective bs = batch_size * accumulate = 16 * 4 = 64
    weights = opt.weights  # initial training weights

    if 'pw' not in opt.arc:  # remove BCELoss positive weights 默认defalutpw,经典的yolo架构
        hyp['cls_pw'] = 1.  # 设置权重
        hyp['obj_pw'] = 1.
    ...
    # Remove previous results清除之前训练产生的batch测试图片和result结果(注意保存!)
    ...
    dataset = LoadImagesAndLabels(train_path,
                                  ...
                                  cache_labels=True if epochs > 10 else False,             # 加载label到内存可以减少对数据的访问耗时
                                  cache_images=False if opt.prebias else opt.cache_images) # 加载图片很耗内存,不推荐
    ...
    model_info(model, report='summary')  # 'full' or 'summary' 打印模型的参数信息
    nb = len(dataloader)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            # imgs:[bs, 3, 416, 416](rect训练时尺度会变)    targets:[num_boxes, 6] (bs内的index0~bs,c,x,y,w,h) paths:图像路径 
            ni = i + nb * epoch  # number integrated batches (since train start) 训练过的总batch数
            imgs = imgs.to(device)
            ...
            pred = model(imgs)
            # Compute loss
            loss, loss_items = compute_loss(pred, targets, model)   # loss_items = (lbox, lobj, lcls, loss)
        ...
        final_epoch = epoch + 1 == epochs   # boolean,判断当前是否为最后一个epoch
        if opt.prebias:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=5000)  # 500200 batches at bs 16, 117263 images = 273 epochs
    ...
    parser.add_argument('--weights', type=str, default='/py/yolov3/weights/darknet53.conv.74', help='initial weights')  # i.e. weights/darknet.53.conv.74
    ...
    # 支持tensorboard终极好评!try成功会生成run文件夹
