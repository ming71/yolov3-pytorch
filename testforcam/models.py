...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):   # p是特征图，img_size是缩放并padding后的尺寸如torch.Size([320, 416])（用来确定原图和特征图的对应位置）
        ...
        if self.training:
            ...
            #   - 一个是将输入reshape分分离出不同类别得分而已    [1, 3, 13, 13, 85]
            # reshape from [1, 3, 13, 13, 85] to [1, 507, 85]
            io_map = io
            return io.view(bs, -1, 5 + self.nc), p , io_map
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):     # x是传入的缩放和padding后的像素矩阵
        ...
        if self.training:
            ...
            # 每个yolo层输出一个2张量的tuple append到output，三个yolo最后的output为[(a1,a2),(b1,b2),(c1,c2)]的形式，unzip后为[(a1,b1,c1),(a2,b2,c2)]
            # [(a1,b1,c1),(a2,b2,c2)]分别是io和p;前者io是3维度每个维度是一张特征图的输出，后者p 5维度
            # inference output因为在yolo层已经将参数经过sigmoid和exp了,并且concat；training output就是原始的特征图未经过任何处理
            io, p, io_map = list(zip(*output))  # inference output, training output
            return torch.cat(io, 1), p, io_map  # 保留bs(1)和数据(5+classes)维度,将中间的proposal进行concat(每个yolo层预测其特征图的w*h*3个proposal)
